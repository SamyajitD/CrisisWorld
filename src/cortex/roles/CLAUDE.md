# src/cortex/roles/ -- Cortex Specialist Roles
...
No imports from `src/env/` or `src/agents/`.
...
| Import | Source | Why |
|--------|--------|-----|
| `RoleProtocol` | `src/protocols/role.py` | All roles implement this |
| `RoleInput`, `Artifact`, `CleanState`, `BeliefState`, `Plan`, `CandidateAction`, `Critique`, `ExecutiveDecision` | `src/schemas/artifact.py` | Role I/O |
| `Observation`, `StakeholderSignal` | `src/schemas/observation.py` | Perception input |
| `RegionState`, `ResourcePool`, `Constraint` | `src/schemas/state.py` | Data within observations |
| `OuterAction` variants | `src/schemas/action.py` | Planner/Executive output |
| `BudgetStatus` | `src/schemas/budget.py` | Executive input |
| `MemoryDigest` | `src/schemas/episode.py` | WorldModeler + Planner input |

**Forbidden:** Nothing from `src/env/`, `src/agents/`, `src/evaluation/`, `src/logging/`.
