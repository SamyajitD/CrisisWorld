# src/cortex/ -- Cortex Deliberation System
...
**Hard rules:**
- Never imports from `src/env/` or `src/agents/`.
...
| Import | Source | Why |
|--------|--------|-----|
| `RoleProtocol` | `src/protocols/role.py` | Each role injected as this type |
| `BudgetProtocol` | `src/protocols/budget.py` | BudgetTracker implements this |
| `MemoryProtocol` | `src/protocols/memory.py` | EpisodeMemory implements this |
| `LoggerProtocol` | `src/protocols/logger.py` | Injected for turn-level logging |
| `Observation` | `src/schemas/observation.py` | Input to deliberation |
| `OuterAction`, `NoOp`, `Escalate` | `src/schemas/action.py` | Output + fallback actions |
| `Artifact`, `CleanState`, `BeliefState`, `Plan`, `Critique`, `ExecutiveDecision`, `RoleInput` | `src/schemas/artifact.py` | Role I/O types |
| `BudgetStatus`, `BudgetExhaustedError` | `src/schemas/budget.py` | Budget tracking |
| `MemoryDigest`, `LogEvent` | `src/schemas/episode.py` | Memory + logging |
