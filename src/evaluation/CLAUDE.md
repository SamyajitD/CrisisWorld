# src/evaluation/ -- Experiment Orchestration
...
imports concrete env/agent classes. Consumed only by `main.py`.
...
**run_episode()**: `obs = env.reset(seed)` -> `agent.reset()` -> loop: `action = agent.act(obs)`, `result = env.step(action)`, `logger.record(...)`, build TurnRecord, accumulate reward -> until done -> finalize trace -> return EpisodeResult.
...
| Import | Source | Why |
|--------|--------|-----|
| `EnvProtocol` | `src/protocols/env.py` | Factory return type |
| `AgentProtocol` | `src/protocols/agent.py` | Factory return type |
| `LoggerProtocol` | `src/protocols/logger.py` | Factory return type |
| `ExperimentConfig` | `src/schemas/config.py` | Configuration |
| `EpisodeResult`, `EpisodeTrace`, `TurnRecord`, `LogEvent` | `src/schemas/episode.py` | Episode data |
| `CompositeReward`, `RewardComponents` | `src/schemas/reward.py` | Metric extraction |
| `BudgetStatus` | `src/schemas/budget.py` | Diagnostic metrics |
| `Observation` | `src/schemas/observation.py` | Turn record |
| `OuterAction` | `src/schemas/action.py` | Turn record |
| `StepResult` | `src/schemas/state.py` | Env step return |
