...
| Import | Source | Why |
|--------|--------|-----|
| `LoggerProtocol` | `src/protocols/logger.py` | EpisodeTracer implements this |
| `EpisodeTrace`, `TurnRecord`, `LogEvent` | `src/schemas/episode.py` | Core data types |
| `Artifact` | `src/schemas/artifact.py` | Logged in turn records |
| `BudgetStatus` | `src/schemas/budget.py` | Budget snapshot per turn |
| `CompositeReward` | `src/schemas/reward.py` | Reward per turn |
| `Observation` | `src/schemas/observation.py` | Observation per turn |
