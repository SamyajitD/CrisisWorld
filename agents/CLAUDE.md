...
**Two-mode operation (OpenEnv):**
- **Training/Evaluation mode**: Agents receive observations directly via `act()` calls
  from the `ExperimentRunner` (current design). This is the primary mode.
- **Production mode** (future): Agents interact with the environment via MCP tools --
  the protocol remains the same but invocation is through MCP, not direct calls.
  `CrisisWorldClient` can be used for remote env interaction.

`Observation` now carries `done`, `reward`, and `metadata` fields (inherited from
OpenEnv base). If `observation.done` is True, agents should return `NoOp`.

**Hard rules:**
- Never imports from `server/` directly.
- Never imports concrete classes from `cortex/`. Protocol types only.
- All wiring in `inference.py`. All stochastic decisions use injected RNG. No `print()`.
...
| Import | Source | Why |
|--------|--------|-----|
| `AgentProtocol` | `protocols.agent` | Both implement this |
| `BudgetProtocol` | `protocols.budget` | CortexAgent budget |
| `LoggerProtocol` | `protocols.logger` | CortexAgent logging |
| `Observation`, `StakeholderSignal` | `models` | Input |
| `OuterAction`, `ActionUnion` + all variants | `models` | Output |
| `EnvConfig` | `models` | FlatAgent config |
| `RegionState`, `ResourcePool` | `models` | FlatAgent reads obs |
| `BudgetStatus`, `BudgetExhaustedError` | `schemas.budget` | CortexAgent error handling |
| `LogEvent` | `schemas.episode` | CortexAgent logging |
...
7. Neither imports from server/ or concrete cortex/
